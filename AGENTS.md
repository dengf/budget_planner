...
`budget-wasm-ocr`, `budget-wasm-pdf`, `budget-wasm-pdfrender` and
`budget-wasm-llm` exist purely to keep `budget-wasm`'s download small, and
to keep each other's weight off a session that only takes one of the
receipt-capture paths. The OCR/PDF split below is the original,
most-documented case of the pattern; `budget-wasm-pdfrender` (rasterizing
a PDF page to pixels via `hayro`, a pure-Rust PDF interpreter) is its
newest instance, added so a scanned PDF (no text layer) has pixels to
read without a text-layer PDF or an image-only session ever paying for
it — see `budget-wasm-pdfrender/src/lib.rs`'s own doc comment.
...
- **`cargo build -p budget-wasm --target wasm32-unknown-unknown` alone
  ...
  `budget-wasm-pdf`, `budget-wasm-pdfrender` and `budget-wasm-llm` are all
  separate crates with separate wasm-pack builds (`npm run build:wasm:ocr`
  / `:pdf` / `:pdfrender` / `:llm`); CI's `wasm32` job checks every one of
  them individually (a real, retroactively-fixed gap: `budget-wasm-llm`
  was added to the crate list without ever being added to this job, and
  ...
  passed that PR without ever building it on wasm32). A local check should too, before
