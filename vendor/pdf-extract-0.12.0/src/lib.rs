        ...
        if let Some(ref unicode_map) = self.unicode_map {
            ...
            let s = match s {
                None => {
                    ...
                    //
                    // budget-planner patch: upstream `.expect()`'d here, panicking (and, under
                    // this workspace's `panic = "abort"` release profile, aborting the whole wasm
                    // module) on a font with neither a unicode-map entry for this char nor any
                    // encoding table at all -- a real bank-statement PDF hit exactly this. The
                    // sibling code path a few lines below (this same match's fallthrough when
                    // `unicode_map` is `None` entirely) already treats a missing encoding as
                    // "fall back to `PDFDocEncoding`" rather than a hard error; this one-line
                    // change makes the two branches consistent instead of one of them panicking.
                    // See vendor/pdf-extract-0.12.0/README.md's patch note for how to re-apply
                    // this against a future upstream version.
                    let encoding = self.encoding.as_ref().map(|x| &x[..]).unwrap_or(&PDFDocEncoding);
