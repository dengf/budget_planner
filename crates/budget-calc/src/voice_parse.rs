...
//!
//! **Why Mandarin/Cantonese tokenize by character, not by whitespace.**
...
//! `Cmn`/`Yue`, `cjk_tokenize` splits into individual Unicode characters
...
// sum. Shared by `Cmn` and `Yue`: written Cantonese numerals use the
// same Han-numeral grammar as Mandarin: only the pronunciation differs.
// ---------------------------------------------------------------------
...
fn extract_amount(words: &[&str], language: VoiceLanguage) -> Option<f64> {
    match language {
        ...
        VoiceLanguage::Cmn | VoiceLanguage::Yue => extract_amount_cjk(words, language),
    }
}
