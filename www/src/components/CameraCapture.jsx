...
export default function CameraCapture({ onFile }) {
  ...
  const capture = () => {
    ...
    setPhase('idle');
    canvas.toBlob(
      (blob) => blob && onFile(new File([blob], 'receipt.jpg', { type: 'image/jpeg' })),
      'image/jpeg',
      0.92,
    );
  };
  ...
  if (phase === 'live') {
    return (
      <div className="camera-live">
        {/* a live viewfinder, not recorded media, so `media-has-caption` does not apply */}
        ...
        <div className="camera-controls">
          ...
          <button
            ...
            onClick={capture}
            aria-label={t('transactions.takePhoto')}
          />
        </div>
      </div>
    );
  ...
  return (
    <>
      ...
      >
        <CameraIcon />
        {phase === 'starting' ? t('transactions.cameraStarting') : t('transactions.takePhoto')}
      </button>
